...
@compute @workgroup_size(8, 8, 1) fn spawnRay(@builtin(global_invocation_id) id: vec3u) {
    ...
    hitRec = traverseTLAS(ray, hitRec);
    hitRec.pos = ray.dir * hitRec.t + ray.orig;
    hitRec.dir = ray.dir;
    ...
    if (hitRec.t < TMAX) {
        ...
        if (ndotl > 0) {
            let shadowRay = Ray(hitRec.pos, hitRec.pixelIndex, lightDirection, 0);
            let shadowt = traverseTLASShadow(shadowRay);
            let incomingLuminance = select(1, 0.33, shadowt < TMAX);
            let color = col * incomingLuminance;
            let corrected = pow(color, vec3f(1.0 / 2.2));
            textureStore(outputTexture, id.xy, vec4f(corrected, 1));
        } else {
            textureStore(outputTexture, id.xy, vec4f(pow(albedo * 0.03, vec3f(1.0 / 2.2)), 1));
        }
    } else {
...
fn traverseTLASShadow(ray: Ray) -> f32 {
    let inverseDir = vec3f(1.0) / ray.dir;
    var t = TMAX;
    var bvhStack = array<u32, 24>();
    var bptr: i32 = 0;
    ...
    while (bptr >= 0) {
        ...
        if (b.triCount == 0) {
            ...
            t_ray.dir = (objInfo.transformInv * vec4f(ray.dir, 0)).xyz;
            t_ray.orig = (objInfo.transformInv * vec4f(ray.orig, 1)).xyz;
            t = traverseBVHShadow(t_ray, objInfo.rootNode);
            if (t < TMAX) {
                return t;
            }
            bptr--;
...
fn traverseBVHShadow(ray: Ray, bvhIdx: u32) -> f32 {
    let inverseDir = vec3f(1.0) / ray.dir;
    var t = TMAX;
    var bvhStack = array<u32, 24>();
    var bptr: i32 = 0;
    ...
    while (bptr >= 0) {
        ...
        if (b.triCount == 0) {
            ...
            for (var i: u32 = 0; i < b.triCount; i++) {
                t = hitTriangleShadow(triangles[i + b.idx], ray);
                if (t < TMAX) {
                    return t;
                }
            }
...
fn hitTriangleShadow(tri: Triangle, ray: Ray) -> f32 {
    var t = TMAX;
    let v1 = vertices[tri.vertices.x];
    let v2 = vertices[tri.vertices.y];
    let v3 = vertices[tri.vertices.z];
